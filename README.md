...
```bash
sudo dnf install -y jq proxychains-ng xdg-utils yad zenity kdialog openssh-clients sshpass telnet nmap-ncat xsel wl-clipboard freerdp remmina keepassxc krdc
```

**Debian / Ubuntu (APT):**
```bash
sudo apt update
sudo apt install -y jq proxychains-ng xdg-utils yad zenity kdialog openssh-client sshpass telnet nmap netcat xsel wl-clipboard freerdp2-x11 remmina keepassxc krdc
```
